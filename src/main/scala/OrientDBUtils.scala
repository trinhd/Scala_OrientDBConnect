...
class OrientDBUtils {
  ...
  val hostAddress = "localhost" //if hostType is "memory:", set it empty
  //val port     = 2424
  val username = "root"
  val password = "12345"
  ...
  def connectDBUsingGraphAPI(): OrientGraphFactory = {
    ...
    val graph: OrientGraph = factory.getTx //get Transaction Instance of Database
    /*
    ...
    try {
      ...
      if (graph.getVertexType(tab_v) == null) {
        ...
        vertexType.createProperty(labelID, OType.STRING)
